...
local function insideArea(obj)
    ...
    return math.abs(pos.X - ESPCenter.X) and math.abs(pos.Y - ESPCenter.Y) and math.abs(pos.Z - ESPCenter.Z) end
...
local function scanWorkspace()
    if _G.God then scanNamesFor(_G.godsSet,"god",Color3.fromRGB(255,0,0)) end
    if _G.Secret then scanNamesFor(_G.secretsSet,"secret",Color3.fromRGB(0,128,255)) end
    if _G.PlayerESP then
...
end)    billboard.AlwaysOnTop = true
    billboard.Adornee = obj
    billboard.Parent = obj
    local tl = Instance.new("TextLabel")
    tl.Size = UDim2.fromScale(1,1)
...
local bestButton = makeButton("ESP Best (toggle)", 160)
...
local best3Button = makeButton("ESP 3 Best (toggle)", 200)
