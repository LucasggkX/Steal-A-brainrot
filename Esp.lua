...
local ESPCenter = Vector3.new(-409, -7, 60)
local ESPRange = 300
...
local function insideArea(obj)
    local pos = getPosition(obj)
    if not pos then return false end
    return math.abs(pos.X - ESPCenter.X) and math.abs(pos.Y - ESPCenter.Y) and math.abs(pos.Z - ESPCenter.Z)
end
...
local function createESP(obj, category, color, label)
    if not obj or ESPMap[obj] or ESPBlacklist[obj] or not obj:IsDescendantOf(workspace) then return end
    if not insideArea(obj) then 
        ESPBlacklist[obj] = true
        destroyESP(obj)
        return 
    end
...
local function updateSpecialESP()
    for obj, esp in pairs(ESPMap) do
        if esp.category == "god" or esp.category == "secret" then
            destroyESP(obj)
        end
    end
            ...
            for _, podium in ipairs(animalPodiums:GetChildren()) do
                ...
                if attach then
                    ...
                    if overhead then
                        ...
                        if stolen and (stolen.Text == "FUSING" or stolen.Text == "IN MACHINE") then
                            continue
                        end
                        local rarity = overhead:FindFirstChild("Rarity")
                        local name = overhead:FindFirstChild("DisplayName")
                        local gen = overhead:FindFirstChild("Generation")
                        if rarity and name and gen then
                            local isGod = rarity.Text == "Brainrot God"
                            local isSecret = rarity.Text == "Secret"
                            if (_G.God and isGod) or (_G.Secret and isSecret) then
                                local color = isGod and Color3.fromRGB(255,0,0) or Color3.fromRGB(0,128,255)
                                local label = string.format("%s | %s\n%s", name.Text, rarity.Text, gen.Text)
                                local billboard = Instance.new("BillboardGui")
                                billboard.Name = "SpecialESP"
                                billboard.AlwaysOnTop = true
                                ...
                                ESPMap[attach] = {category = isGod and "god" or "secret", gui = billboard}
                            end
                        end
                    end
                end
...
titleLabel.Text = "Esp: Made By Lucas"
...
local bestButton = makeButton("ESP Best Brainrot (toggle)", 160)
local best3Button = makeButton("ESP 3 Best Brainrot (toggle)", 200)
...
bestButton.MouseButton1Click:Connect(function()
    _G.bestESPEnabled = not _G.bestESPEnabled
    setBtnState(bestButton,_G.bestESPEnabled)
end)
task.spawn(function()
    local currentESP = nil
    while task.wait(1) do
        if _G.bestESPEnabled then
            local function parseValue(str)
                str = str:gsub("%$", ""):gsub("/s", "")
                local num, suf = str:match("([%d%.]+)([MK]?)")
                num = tonumber(num) or 0
                if suf == "K" then num = num * 1_000
                elseif suf == "M" then num = num * 1_000_000 end
                return num
            end
            ...
            if bestAnimal then
                ...
                textLabel.TextScaled = true
                textLabel.Font = Enum.Font.SourceSansBold
                textLabel.Text = string.format("%s | %s\n%s", bestAnimal.nome, bestAnimal.raridade, bestAnimal.porSegundo)
                textLabel.Parent = billboard
                currentESP = billboard
            end
        elseif currentESP then
            currentESP:Destroy()
            currentESP = nil
        end
    end
end)
best3Button.MouseButton1Click:Connect(function()
    _G.best3ESPEnabled = not _G.best3ESPEnabled
    setBtnState(best3Button,_G.best3ESPEnabled)
end)
task.spawn(function()
    local currentESPs = {}
    while task.wait(0.5) do
        if _G.best3ESPEnabled then
            local function parseValue(str)
                str = str:gsub("%$", ""):gsub("/s", "")
                local num, suf = str:match("([%d%.]+)([MK]?)")
                num = tonumber(num) or 0
                                    ...
                                    if overhead then
                                        ...
                                        if gen and rarity and name then
                                            table.insert(allAnimals,{
                                                ...
                                                raridade = rarity.Text,
                                                porSegundo = gen.Text,
                                                value = tonumber(parseValue(gen.Text))
                                            })
                                        end
                                    end
                                end
            ...
            table.sort(allAnimals,function(a,b) return a.value>b.value end)
            local top3 = {allAnimals[1], allAnimals[2], allAnimals[3]}
            for _,esp in ipairs(currentESPs) do
                if esp then esp:Destroy() end
            end
            currentESPs = {}
            for _, animal in ipairs(top3) do
                if animal then
                    local billboard = Instance.new("BillboardGui")
                    billboard.Name = "BestAnimalESP"
                    billboard.AlwaysOnTop = true
                    billboard.Size = UDim2.new(0,200,0,50)
                    billboard.StudsOffset = Vector3.new(0,3,0)
                    billboard.Parent = animal.attachment
                    local textLabel = Instance.new("TextLabel")
                    textLabel.Size = UDim2.new(1,0,1,0)
                    textLabel.BackgroundTransparency = 1
                    textLabel.TextColor3 = Color3.new(1,1,0)
                    textLabel.TextScaled = true
                    textLabel.Font = Enum.Font.SourceSansBold
                    textLabel.Text = string.format("%s | %s\n%s", animal.nome, animal.raridade, animal.porSegundo)
                    textLabel.Parent = billboard
                    table.insert(currentESPs,billboard)
                end
            end
        else
            for _,esp in ipairs(currentESPs) do
